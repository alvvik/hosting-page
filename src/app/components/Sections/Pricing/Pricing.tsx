...
const offers: OfferProp[] = [
  ...
  },
  {
    id: 3,
    name: "Obsidian",
    description: "Doskonały dla bardzo dużych modpacków jak ATM10 lub Better Minecraft",
    price: 47.99,
    cpu: 4,
    ram: 9,
    disk: 35
...
export default function Pricing(){
  return (
    <section className="flex flex-col items-center justify-center gap-4 px-0 py-8 ">
      <h3 className="text-fluid-h3 font-semibold">Nasza oferta</h3>
      <p className="text-fluid-small text-text-muted text-center">
        Wybierz plan, który najlepiej odpowiada Twoim potrzebom i ciesz się pełną kontrolą nad swoim serwerem.
      </p>
      <div className="grid gap-4 p-4 md:grid-cols-3 md:gap-6 md:max-w-5xl md:mx-auto md:items-start">
        {offers.map((offer) => (
         <div key={offer.id} className="relative flex flex-col">
          ...
          <div className={`bg-surface flex flex-col gap-2 p-4 rounded-2xl shadow ${offer.isPopular ? 'ring-2 ring-cta' : ''}`}>
            <p className="font-bold text-fluid-lead" >{offer.name}</p>
            <p className="text-fluid-small text-text-muted">{offer.description}</p>
            <p>
              <span className="font-bold text-fluid-h2">{offer.price.toFixed(2)}zł</span> <br /> za miesiąc
            </p>
            <div>
